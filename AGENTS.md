...
### 请求-响应原语
事件的分发模式由 `agent/core/events.py` 的 `EVENT_MODES` 登记表决定（`parallel` 并发观察 / `serial` 顺序执行、首个非 None 短路并作为 emit 返回值）。未登记的事件按 `parallel` 分发并打 warning 日志。模式是事件契约的一部分：新事件应登记；监听方可从 `evt.mode` 读取当前模式。emit 用 `asyncio.gather` 等待所有 handler 完成，单个 handler 异常被隔离记日志，不中断其他 handler 与 emit 本身。
