...
- **工具** (`tool.py`)：可执行工具的抽象基类和注册表，从 `agent/tools/` 加载 Tool 子类，传入 AgentLoop 供 LLM 调用。`config.yml` 中 tools 列表支持字符串或带参数的 dict 格式，参数通过 `Tool.config` 传递给工具实例
...
**风格**
- 编码风格要保持一致（同样是响应事件，不能有的是 on_xxx, 有的是 handle_xxx；变量名不能有的驼峰，有的下划线）
...
- commit message 格式：`类型: 简要概括改动内容`；commit 详情用列表格式逐行列出主要改动点，不要罗列代码；内容语言和历史记录一致

**方法**
- 定位问题时，在必要的输入、输出、关键节点添加足够的日志，以满足全流程自动化验证的要求
- 要先构造条件相同的用例复现问题，严禁不复现凭猜测直接出方案
...
- 不需要的代码（包括死代码）、目录、配置项、临时脚本都要清除干净
