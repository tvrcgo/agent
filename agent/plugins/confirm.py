...
from agent.core.loop import JobAborted, JobContext
from agent.core.ws import StatusEvent


class ConfirmPlugin(Plugin):
    """Intercepts request_confirmation tool calls, waits for user approve/deny."""
    ...
    async def _on_before_tool(self, ctx: JobContext) -> None:
    ...
    async def _on_command_confirm(self, ctx: JobContext) -> None:
