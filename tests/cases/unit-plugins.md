...
- [ ] `load(ctx, config)` 注册 `tool_start`、`request_confirm`、`cmd_confirm` 三个事件
- [ ] `_on_tool_start`：非 `request_confirmation` 工具调用直接放过
- [ ] `_on_tool_start`：匹配时阻塞等待 `cmd_confirm` 唤醒
- [ ] `_on_request_confirm`：向 `job.output` 追加 `confirm_request` 事件并等待决策，结果回填 `event.data["confirm_decision"]`
- [ ] `cmd_confirm` deny → `_on_tool_start` 在 `event.data` 写 `abort=True` + 取消消息，工具跳过执行
